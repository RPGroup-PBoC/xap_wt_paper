...
if __name__ == "__main__":
    ...
    smod = stochpy.SSA()   
    smod.Model("Xap_System_3d_bursts.psc",dir_path)
    smod.Method("Tauleap")
    #smod.SetSeeding(10)
    
    params = [['rho_m_burst',25 * 1.0e-9 * 1.0e-3 * 1.0e9 / 3.211610794067383],['gamma_m ',5 * 1.0e-3],['r_p ',5.0e-4 * 1.0e2],
                   ...
                   ['KxI', 5.0e-5 * 1.0e4 * 1.0e9],['expEx',150],['expEc',150]] 
    initials = [['mRNA',0],['protein',0],['xanth', 0]]  
    
#    start = time.time()
#    find_distribution_data(smod,params,initials,200,1.0e6)
#    end = time.time()
#    print(end - start)
    
#    print(switch_time_data(smod,params,initials,[50*5.0e4, 70*5.0e4],2,[1.0e6,1.0e5],[400,400]))
##    
...
#        data_writer.writerows([[item[0],item[1]] for item in writingdata])
    ...
    param_export = [(' ' + params[i][0] + ': ' + str(params[i][1])) for i in range(len(params))]
    init_export = [(' ' + initials[i][0] + ': ' + str(initials[i][1])) for i in range(len(initials))]
    writingdata = find_switch_time(smod,runs,9.0e5,452)
    with open(dir_path + '\\Output\\TimeDistribution\\' + str(datetime.now().strftime("%Y-%m-%d %H-%M-%S")) + '.csv', mode='w', newline='') as csv_file:
        data_writer = csv.writer(csv_file, delimiter=',')
        data_writer.writerow(["Adaptation time distribution from " + str(runs) + " runs (with bursts)"])
        data_writer.writerow(['parameter values: '])
        data_writer.writerows([param_export])
        data_writer.writerow(['initial values: '])
        data_writer.writerows([init_export])
