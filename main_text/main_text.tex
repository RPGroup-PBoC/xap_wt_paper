...
\begin{document}
	...
	\section*{Introduction} \todo{remove images, todonotes, and funding for submission}
	...
	rate equal to zero. Experiments show that the expression becomes very weak when one of
	...
	For the Nup transporters, we also use Michaelis-Menten kinetics but can linearize the Michaelis-Menten term $k_{\n{cat}} \frac{\n{[x]}}{K_{\n{M}} + \n{[x]}} \approx \tilde{k} \n{[x]}$ because of the transporter's low affinity for xanthosine (i.e. $K_{\n{M}} \gg \n{[x]}$). For the rate parameters $\tilde{k}$, we write $k_{\n{nup}}$ for
	influx and $\xi k_{\n{nup}}$ for efflux. 
	...
	expression, the copy numbers in the problem vary from less than five to several thousand.
	The larger the amounts of mRNA, protein, and xanthosine, the larger the number of reactions, and thus, the larger the computational cost of Gillespie's classical algorithm. This makes Gillespie's \texttau-leap algorithm ideal for the high expression state. On the
	other hand, \texttau-leaping cannot be used for the small copy numbers in the low expression state. For
	...
	should be taken with reservation. Cell divisions are not considered here,
	...
	xanthosine. The value we found is roughly one to two orders of magnitude larger than what has been measured for LacI and IPTG~\cite{RazoMejia2018}, meaning the interaction of XapR and xanthosine is rather weak.
	...
	simulations showed that deterministic bistability does not mean that
	...
	This work was supported by 1R35 GM118043-01 Maximizing Investigators' Research Award (MIRA) (to R.P.), the National Science Foundation
	Graduate Research Fellowship under Grant No.\ DGE-114446 (M.J.M.). and the
	Werner Siemens Foundation through the Swiss Study Foundation (K.S.L.).
