...
\todo[inline]{MJM: Are the following 2 paragraphs excessively detailed for main text?
maybe replace w/ 1-2 sentences about low affinity Mich-Ment transporters
that supplement XapB, \& shift all this to supplement?
If we want to emphasize the general class of model rather than the particular
biology details of Xap}
...
\todo{MJM: Do any of the older papers have data for this too,
or were they all just colony growth screens on plates?}
...
\todo{MJM: the (hypothetical) utility of this switch seems to be resource efficiency,
which $\Rightarrow$ pushing the fixed points to extreme highs \& lows is good
$\Rightarrow$ stronger coop is good. maybe we should work this in}
...
\todo{MJM: but that already sounds like an effective interaction,
I'm not sure I like this phrasing, or the next sentence's.}
...
\todo{MJM: depends on an effective barrier height, hard to intuit from phase port.
Bistability rather than bimodality means it may very well practically never switch.}
...
% state (recall $\n{[c]_{a}} \defeq \frac{c}{K_{\n{a}}}$, $K_{\n{a}} = 5 \cdot
% 10^{-5} \unit{M}$).
\todo[inline]{Is this understandable without seeing the data? Do
you have any suggestions to make it clearer?
MJM: I think the quantitative details can go to the supplement where they
have more context, the qualitative observation of hysteresis is enough here.}
...
features.
\todo[inline]{MJM: I think this last sentence sells us short: qualitative changes result
if we remove any present feature of the model.
That sounds to me like a distillation of the essential.}
...
\todo[inline]{MJM: I'm not sure we've really proven the asserted points re 3D \& Hill.
I would advocate removing this whole paragraph.}
...
variable and parameter \todo{MJM: I'm confused,
we estimated all the model parameters, so what expectations are we comparing to?}
...
\todo{Is the conclusion too long? MJM: not necessarily}
...
\todo[inline]{MJM: Software/code acknowledgements usually go in a separate section,
and if we want we could set up a code repo to cite along with that.}
...
\todo[inline]{Should I also list the references that are cited in the appendix? MJM: no, I think the supplement has its own separate list.}

\todo[inline]{The capitalization style is different for the titles of the references. Should I list them as they are on the corresponding paper or should I do it uniformly? Does it matter at all? Otherwise I leave it as it is. MJM: I think the copy editors will sort it out.}
