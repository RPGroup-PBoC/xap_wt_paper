...
switches. They can be constructed in various ways and have different
functions. What all of them have in common is that the system is bistable:
there are two stable steady-states that it can be in. The distribution of
...
but the gene expression in each cell is fluctuating between the two levels.
...
Fig~\ref{fig1:switches}). Here, the two stable states are high expression of
the first protein and low expression of the second one or the opposite. This
...
\begin{figure}%[h!]
	...
	(A) and (B) show the two most well-known genetic switches that are
	mentioned in the text. In (C), a very much simplified version of the
...
circuit is made up of two genes: one that encodes for XapR and another one
that encodes for XapA and XapB. XapR appears to be a transcription factor
that is induced by xanthosine and activates the \emph{xapAB} promoter. It
seems to have two binding sites on the promoter, but the promoter
architecture and function is not fully understood up to now. The
